...
// Page object
$AcademicpublicEdit = &$Page;
?>
<script>
if (!ew.vars.tables.academicpublic) ew.vars.tables.academicpublic = <?= JsonEncode(GetClientVar("tables", "academicpublic")) ?>;
var currentForm, currentPageID;
var facademicpublicedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    facademicpublicedit = currentForm = new ew.Form("facademicpublicedit", "edit");

    // Add fields
    var fields = ew.vars.tables.academicpublic.fields;
    facademicpublicedit.addFields([
        ["Public_Id", [fields.Public_Id.required ? ew.Validators.required(fields.Public_Id.caption) : null], fields.Public_Id.isInvalid],
    ...
    // Validate form
    facademicpublicedit.validate = function () {
    ...
    // Form_CustomValidate
    facademicpublicedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    facademicpublicedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    facademicpublicedit.lists.Aca_Id = <?= $Page->Aca_Id->toClientList($Page) ?>;
    facademicpublicedit.lists.Public_Type = <?= $Page->Public_Type->toClientList($Page) ?>;
    loadjs.done("facademicpublicedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="facademicpublicedit" id="facademicpublicedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Public_Id->Visible) { // Public_Id ?>
    <div id="r_Public_Id" class="form-group row">
        <label id="elh_academicpublic_Public_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Public_Id->caption() ?><?= $Page->Public_Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Public_Id->cellAttributes() ?>>
<span id="el_academicpublic_Public_Id">
<span<?= $Page->Public_Id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Public_Id->getDisplayValue($Page->Public_Id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="academicpublic" data-field="x_Public_Id" data-hidden="1" name="x_Public_Id" id="x_Public_Id" value="<?= HtmlEncode($Page->Public_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["facademicpublicedit", "datetimepicker"], function() {
    ew.createDateTimePicker("facademicpublicedit", "x_Public_Date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
