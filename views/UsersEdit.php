...
loadjs.ready("head", function () {
    ...
    fusersedit.addFields([
        ["Users__Id", [fields.Users__Id.required ? ew.Validators.required(fields.Users__Id.caption) : null], fields.Users__Id.isInvalid],
    ...
    fusersedit.lists.Per_Id = <?= $Page->Per_Id->toClientList($Page) ?>;
...
<?php if ($Page->Users__Id->Visible) { // Users _Id ?>
    <div id="r_Users__Id" class="form-group row">
        <label id="elh_users_Users__Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Users__Id->caption() ?><?= $Page->Users__Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Users__Id->cellAttributes() ?>>
<span id="el_users_Users__Id">
<span<?= $Page->Users__Id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Users__Id->getDisplayValue($Page->Users__Id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="users" data-field="x_Users__Id" data-hidden="1" name="x_Users__Id" id="x_Users__Id" value="<?= HtmlEncode($Page->Users__Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<div class="input-group">
    <input type="password" name="x_Users_Password" id="x_Users_Password" autocomplete="new-password" data-field="x_Users_Password" value="<?= $Page->Users_Password->EditValue ?>" size="30" maxlength="100" placeholder="<?= HtmlEncode($Page->Users_Password->getPlaceHolder()) ?>"<?= $Page->Users_Password->editAttributes() ?> aria-describedby="x_Users_Password_help">
