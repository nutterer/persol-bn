...
loadjs.ready("head", function () {
    ...
    // Validate form
    fpersonnelplanedit.validate = function () {
    ...
    // Form_CustomValidate
    fpersonnelplanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpersonnelplanedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fpersonnelplanedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpersonnelplanedit" id="fpersonnelplanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Plan_Id->Visible) { // Plan_Id ?>
    <div id="r_Plan_Id" class="form-group row">
        <label id="elh_personnelplan_Plan_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Plan_Id->caption() ?><?= $Page->Plan_Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="personnelplan" data-field="x_Plan_Id" data-hidden="1" name="x_Plan_Id" id="x_Plan_Id" value="<?= HtmlEncode($Page->Plan_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" name="fa_x_Plan_File" id= "fa_x_Plan_File" value="<?= (Post("fa_x_Plan_File") == "0") ? "0" : "1" ?>">
