...
// Page object
$GradAdmissionEdit = &$Page;
?>
<script>
if (!ew.vars.tables.grad_admission) ew.vars.tables.grad_admission = <?= JsonEncode(GetClientVar("tables", "grad_admission")) ?>;
var currentForm, currentPageID;
var fgrad_admissionedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fgrad_admissionedit = currentForm = new ew.Form("fgrad_admissionedit", "edit");

    // Add fields
    var fields = ew.vars.tables.grad_admission.fields;
    fgrad_admissionedit.addFields([
        ["Grad_Admission_id", [fields.Grad_Admission_id.required ? ew.Validators.required(fields.Grad_Admission_id.caption) : null], fields.Grad_Admission_id.isInvalid],
    ...
    // Validate form
    fgrad_admissionedit.validate = function () {
    ...
    // Form_CustomValidate
    fgrad_admissionedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fgrad_admissionedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fgrad_admissionedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fgrad_admissionedit" id="fgrad_admissionedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Grad_Admission_id->Visible) { // Grad_Admission_id ?>
    <div id="r_Grad_Admission_id" class="form-group row">
        <label id="elh_grad_admission_Grad_Admission_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Grad_Admission_id->caption() ?><?= $Page->Grad_Admission_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Grad_Admission_id->cellAttributes() ?>>
<span id="el_grad_admission_Grad_Admission_id">
<span<?= $Page->Grad_Admission_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Grad_Admission_id->getDisplayValue($Page->Grad_Admission_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="grad_admission" data-field="x_Grad_Admission_id" data-hidden="1" name="x_Grad_Admission_id" id="x_Grad_Admission_id" value="<?= HtmlEncode($Page->Grad_Admission_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
