...
loadjs.ready("head", function () {
    ...
    fawardedit = currentForm = new ew.Form("fawardedit", "edit");

    // Add fields
    var fields = ew.vars.tables.award.fields;
    fawardedit.addFields([
        ["Award_Id", [fields.Award_Id.required ? ew.Validators.required(fields.Award_Id.caption) : null], fields.Award_Id.isInvalid],
    ...
    // Form_CustomValidate
    fawardedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fawardedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fawardedit.lists.Per_Id = <?= $Page->Per_Id->toClientList($Page) ?>;
    loadjs.done("fawardedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fawardedit" id="fawardedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Award_Id->Visible) { // Award_Id ?>
    <div id="r_Award_Id" class="form-group row">
        <label id="elh_award_Award_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Award_Id->caption() ?><?= $Page->Award_Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Award_Id->cellAttributes() ?>>
<span id="el_award_Award_Id">
<span<?= $Page->Award_Id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Award_Id->getDisplayValue($Page->Award_Id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="award" data-field="x_Award_Id" data-hidden="1" name="x_Award_Id" id="x_Award_Id" value="<?= HtmlEncode($Page->Award_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
