...
loadjs.ready("head", function () {
    ...
    // Validate form
    fselfdevelopmentedit.validate = function () {
    ...
    // Form_CustomValidate
    fselfdevelopmentedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fselfdevelopmentedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fselfdevelopmentedit.lists.Per_Id = <?= $Page->Per_Id->toClientList($Page) ?>;
    loadjs.done("fselfdevelopmentedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fselfdevelopmentedit" id="fselfdevelopmentedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->SelfDev_Id->Visible) { // SelfDev_Id ?>
    <div id="r_SelfDev_Id" class="form-group row">
        <label id="elh_selfdevelopment_SelfDev_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->SelfDev_Id->caption() ?><?= $Page->SelfDev_Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="selfdevelopment" data-field="x_SelfDev_Id" data-hidden="1" name="x_SelfDev_Id" id="x_SelfDev_Id" value="<?= HtmlEncode($Page->SelfDev_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fselfdevelopmentedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fselfdevelopmentedit", "x_SelfDev_StartDate", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fselfdevelopmentedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fselfdevelopmentedit", "x_SelfDev_EndDate", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
