...
?>
<script>
if (!ew.vars.tables._05report) ew.vars.tables._05report = <?= JsonEncode(GetClientVar("tables", "_05report")) ?>;
...
loadjs.ready("head", function () {
    ...
    f_05reportedit = currentForm = new ew.Form("f_05reportedit", "edit");

    // Add fields
    var fields = ew.vars.tables._05report.fields;
    f_05reportedit.addFields([
    ...
    // Validate form
    f_05reportedit.validate = function () {
    ...
    // Form_CustomValidate
    f_05reportedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    f_05reportedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("f_05reportedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="f_05reportedit" id="f_05reportedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Report_Id->Visible) { // Report_Id ?>
    <div id="r_Report_Id" class="form-group row">
        <label id="elh__05report_Report_Id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Report_Id->caption() ?><?= $Page->Report_Id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Report_Id->cellAttributes() ?>>
<span id="el__05report_Report_Id">
...
<input type="hidden" data-table="_05report" data-field="x_Report_Id" data-hidden="1" name="x_Report_Id" id="x_Report_Id" value="<?= HtmlEncode($Page->Report_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
<?php if ($Page->Report_Year->Visible) { // Report_Year ?>
    <div id="r_Report_Year" class="form-group row">
        <label id="elh__05report_Report_Year" for="x_Report_Year" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Report_Year->caption() ?><?= $Page->Report_Year->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Report_Year->cellAttributes() ?>>
<span id="el__05report_Report_Year">
<input type="<?= $Page->Report_Year->getInputTextType() ?>" data-table="_05report" data-field="x_Report_Year" name="x_Report_Year" id="x_Report_Year" size="30" maxlength="4" placeholder="<?= HtmlEncode($Page->Report_Year->getPlaceHolder()) ?>" value="<?= $Page->Report_Year->EditValue ?>"<?= $Page->Report_Year->editAttributes() ?> aria-describedby="x_Report_Year_help">
...
<?php if ($Page->Report_File->Visible) { // Report_File ?>
    <div id="r_Report_File" class="form-group row">
        <label id="elh__05report_Report_File" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Report_File->caption() ?><?= $Page->Report_File->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Report_File->cellAttributes() ?>>
<span id="el__05report_Report_File">
<div id="fd_x_Report_File">
<div class="input-group">
    <div class="custom-file">
        <input type="file" class="custom-file-input" title="<?= $Page->Report_File->title() ?>" data-table="_05report" data-field="x_Report_File" name="x_Report_File" id="x_Report_File" lang="<?= CurrentLanguageID() ?>"<?= $Page->Report_File->editAttributes() ?><?= ($Page->Report_File->ReadOnly || $Page->Report_File->Disabled) ? " disabled" : "" ?> aria-describedby="x_Report_File_help">
