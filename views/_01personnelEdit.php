...
loadjs.ready("head", function () {
    ...
    // Validate form
    f_01personneledit.validate = function () {
    ...
    // Form_CustomValidate
    f_01personneledit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    f_01personneledit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    f_01personneledit.lists.Per_Type = <?= $Page->Per_Type->toClientList($Page) ?>;
    f_01personneledit.lists.Per_EmployeeType = <?= $Page->Per_EmployeeType->toClientList($Page) ?>;
    f_01personneledit.lists.Per_Position = <?= $Page->Per_Position->toClientList($Page) ?>;
    f_01personneledit.lists.Per_major = <?= $Page->Per_major->toClientList($Page) ?>;
    f_01personneledit.lists.Per_Academic = <?= $Page->Per_Academic->toClientList($Page) ?>;
    f_01personneledit.lists.Per_Administrative = <?= $Page->Per_Administrative->toClientList($Page) ?>;
    f_01personneledit.lists.Per_Nationality = <?= $Page->Per_Nationality->toClientList($Page) ?>;
    f_01personneledit.lists.Per_Religion = <?= $Page->Per_Religion->toClientList($Page) ?>;
    f_01personneledit.lists.Per_WorkStatus = <?= $Page->Per_WorkStatus->toClientList($Page) ?>;
    loadjs.done("f_01personneledit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="f_01personneledit" id="f_01personneledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="_01personnel" data-field="x_Per_Id" data-hidden="1" name="o_Per_Id" id="o_Per_Id" value="<?= HtmlEncode($Page->Per_Id->OldValue ?? $Page->Per_Id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["f_01personneledit", "datetimepicker"], function() {
    ew.createDateTimePicker("f_01personneledit", "x_Per_WorDateStart", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["f_01personneledit", "datetimepicker"], function() {
    ew.createDateTimePicker("f_01personneledit", "x_Per_WorkDate", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["f_01personneledit", "datetimepicker"], function() {
    ew.createDateTimePicker("f_01personneledit", "x_Per_Born", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
