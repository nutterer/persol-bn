...
// Page object
$PerPositionEdit = &$Page;
?>
<script>
if (!ew.vars.tables.per_position) ew.vars.tables.per_position = <?= JsonEncode(GetClientVar("tables", "per_position")) ?>;
var currentForm, currentPageID;
var fper_positionedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fper_positionedit = currentForm = new ew.Form("fper_positionedit", "edit");

    // Add fields
    var fields = ew.vars.tables.per_position.fields;
    fper_positionedit.addFields([
        ["Per_Position_id", [fields.Per_Position_id.required ? ew.Validators.required(fields.Per_Position_id.caption) : null], fields.Per_Position_id.isInvalid],
    ...
    // Validate form
    fper_positionedit.validate = function () {
    ...
    // Form_CustomValidate
    fper_positionedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fper_positionedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fper_positionedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fper_positionedit" id="fper_positionedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->Per_Position_id->Visible) { // Per_Position_id ?>
    <div id="r_Per_Position_id" class="form-group row">
        <label id="elh_per_position_Per_Position_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Per_Position_id->caption() ?><?= $Page->Per_Position_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->Per_Position_id->cellAttributes() ?>>
<span id="el_per_position_Per_Position_id">
<span<?= $Page->Per_Position_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Per_Position_id->getDisplayValue($Page->Per_Position_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="per_position" data-field="x_Per_Position_id" data-hidden="1" name="x_Per_Position_id" id="x_Per_Position_id" value="<?= HtmlEncode($Page->Per_Position_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
