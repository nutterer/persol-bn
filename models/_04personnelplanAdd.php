 ...
 * Page class
 */
class _04personnelplanAdd extends _04personnelplan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "_04personnelplanAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("_04personnelplanList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "_04personnelplanList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "_04personnelplanView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->Plan_Id->OldValue = $this->Plan_Id->CurrentValue;
        $this->Plan_Year->CurrentValue = null;
        $this->Plan_Year->OldValue = $this->Plan_Year->CurrentValue;
        $this->Plan_File->CurrentValue = null;
        $this->Plan_File->OldValue = $this->Plan_File->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Plan_File' first before field var 'x_Plan_File'
        $val = $CurrentForm->hasValue("Plan_File") ? $CurrentForm->getValue("Plan_File") : $CurrentForm->getValue("x_Plan_File");
        if (!$this->Plan_File->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Plan_File->Visible = false; // Disable update for API request
            } else {
                $this->Plan_File->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->Plan_File->CurrentValue = $this->Plan_File->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['Plan_Id'] = $this->Plan_Id->CurrentValue;
        $row['Plan_Year'] = $this->Plan_Year->CurrentValue;
        $row['Plan_File'] = $this->Plan_File->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->Plan_File->EditValue = HtmlEncode($this->Plan_File->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->Plan_File->Required) {
            if (!$this->Plan_File->IsDetailKey && EmptyValue($this->Plan_File->FormValue)) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Plan_Year
        $this->Plan_Year->setDbValueDef($rsnew, $this->Plan_Year->CurrentValue, "", false);

        // Plan_File
        $this->Plan_File->setDbValueDef($rsnew, $this->Plan_File->CurrentValue, "", false);
