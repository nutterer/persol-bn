...
class Users extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // Users _Id
        $this->Users__Id = new DbField('users', 'users', 'x_Users__Id', 'Users _Id', '`Users _Id`', '`Users _Id`', 3, 5, -1, false, '`Users _Id`', false, false, false, 'FORMATTED TEXT', 'NO');
        $this->Users__Id->IsAutoIncrement = true; // Autoincrement field
        $this->Users__Id->IsPrimaryKey = true; // Primary key field
        $this->Users__Id->Sortable = true; // Allow sort
        $this->Users__Id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['Users _Id'] = &$this->Users__Id;

        // Per_Id
        $this->Per_Id = new DbField('users', 'users', 'x_Per_Id', 'Per_Id', '`Per_Id`', '`Per_Id`', 3, 10, -1, false, '`Per_Id`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        // Users_Name
        $this->Users_Name = new DbField('users', 'users', 'x_Users_Name', 'Users_Name', '`Users_Name`', '`Users_Name`', 200, 50, -1, false, '`Users_Name`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->Users_Name->Nullable = false; // NOT NULL field
        $this->Users_Name->Required = true; // Required field
        $this->Users_Name->Sortable = true; // Allow sort
        $this->Fields['Users_Name'] = &$this->Users_Name;

        // Users_Password
        $this->Users_Password = new DbField('users', 'users', 'x_Users_Password', 'Users_Password', '`Users_Password`', '`Users_Password`', 200, 100, -1, false, '`Users_Password`', false, false, false, 'FORMATTED TEXT', 'PASSWORD');
        $this->Users_Password->Nullable = false; // NOT NULL field
        $this->Users_Password->Required = true; // Required field
        $this->Users_Password->Sortable = true; // Allow sort
        $this->Fields['Users_Password'] = &$this->Users_Password;

        // Users_Permission
        $this->Users_Permission = new DbField('users', 'users', 'x_Users_Permission', 'Users_Permission', '`Users_Permission`', '`Users_Permission`', 200, 100, -1, false, '`Users_Permission`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        $this->Users_Permission->Nullable = false; // NOT NULL field
        $this->Users_Permission->Required = true; // Required field
        $this->Users_Permission->Sortable = true; // Allow sort
        $this->Fields['Users_Permission'] = &$this->Users_Permission;
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($success) {
            ...
            $this->Users__Id->setDbValue($conn->lastInsertId());
            $rs['Users _Id'] = $this->Users__Id->DbValue;
        }
        return $success;
    }
    ...
    protected function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Users _Id', $rs)) {
                AddFilter($where, QuotedName('Users _Id', $this->Dbid) . '=' . QuotedValue($rs['Users _Id'], $this->Users__Id->DataType, $this->Dbid));
            }
        }
        $filter = ($curfilter) ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Users__Id->DbValue = $row['Users _Id'];
        $this->Per_Id->DbValue = $row['Per_Id'];
        $this->Users_Name->DbValue = $row['Users_Name'];
        $this->Users_Password->DbValue = $row['Users_Password'];
        $this->Users_Permission->DbValue = $row['Users_Permission'];
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Users__Id->CurrentValue : $this->Users__Id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Users__Id->CurrentValue = $keys[0];
            } else {
                $this->Users__Id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Users _Id', $row) ? $row['Users _Id'] : null;
        } else {
            $val = $this->Users__Id->OldValue !== null ? $this->Users__Id->OldValue : $this->Users__Id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Users__Id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    {
        $json = "";
        $json .= "Users__Id:" . JsonEncode($this->Users__Id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Users__Id->CurrentValue !== null) {
            $url .= "/" . rawurlencode($this->Users__Id->CurrentValue);
        } else {
...
SORTHTML;
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            if (($keyValue = Param("Users__Id") ?? Route("Users__Id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Users__Id->CurrentValue = $key;
            } else {
                $this->Users__Id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->Users_Name->EditValue = $this->Users_Name->CurrentValue;
        ...
        $this->Users_Permission->EditValue = $this->Users_Permission->CurrentValue;
